...
class BillExportService {
  static Future<void> exportAsPdf(BillGenerateReport report, {String? billType}) async {
  ...
  static List<pw.Widget> _buildRegularPdfBody(BillGenerateReport report) {
    ...
    tradeData.add(_buildPdfRegularRow(['EXCHANGE', 'SCRIPT', 'BUY QTY', 'BUY PRICE', 'SELL QTY', 'SELL PRICE', 'BROKERAGE', 'PROFIT/LOSS'], isHeader: true));
    ...
    for (final trade in report.scriptTrades) {
      ...
      for (int i = 0; i < maxLen; i++) {
        ...
        tradeData.add(_buildPdfRegularRow([
          trade.exchange,
          trade.script,
          buyLeg?.qty.toString() ?? '',
          buyLeg?.price ?? '',
          ...
          PdfColor.fromHex('B91C1C'), 
          null,
          trade.profitLoss >= 0 ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
        ]));
      }
    }
    ...
    tradeData.add(_buildPdfRegularRow([
      ...
      report.summaryTotal.total >= 0 ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
    ]));
    bodyItems.add(pw.Container(
    ...
    scriptSummaryRows.add(_buildPdfRegularRow(['EXCHANGE', 'SCRIPT', 'MTM', 'BROKERAGE', 'NET AMOUNT'], isHeader: true));
    
    for (final s in report.scriptWiseSummary) {
      scriptSummaryRows.add(_buildPdfRegularRow([
        s.exchange,
        s.script,
        s.total.toStringAsFixed(2),
        s.brokerage.toStringAsFixed(2),
        ...
        null, null,
        s.total >= 0 ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
        null,
        s.net >= 0 ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
      ]));
    }
    ...
    scriptSummaryRows.add(_buildPdfRegularRow([
      ...
      report.summaryTotal.total >= 0 ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
      null,
      report.summaryTotal.totalNet >= 0 ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
    ]));
    bodyItems.add(pw.Container(
      decoration: pw.BoxDecoration(border: pw.TableBorder.all(color: PdfColors.grey300, width: 0.5)),
    ...
    if (report.carryForward.isNotEmpty) {
      ...
      cfRows.add(_buildPdfRegularRow(['EXCHANGE', 'SCRIPT', 'POSITION', 'QTY', 'RATE'], isHeader: true));
      for (final cf in report.carryForward) {
        cfRows.add(_buildPdfRegularRow([
          cf.exchange,
          cf.script,
          cf.type.toUpperCase(),
          ...
          null, null,
          cf.type.toLowerCase() == 'buy' ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
          null,
          PdfColor.fromHex('1E40AF'),
        ]));
      }
    ...
    if (report.exchangeWisePL.isNotEmpty) {
      ...
      exSummaryRows.add(_buildPdfRegularRow(['EXCHANGE', 'MTM', 'BROKERAGE', 'NET AMOUNT'], isHeader: true));
      for (final pl in report.exchangeWisePL) {
        exSummaryRows.add(_buildPdfRegularRow([
          pl.exchange,
          ...
          null,
          pl.mtm >= 0 ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
          null,
          pl.pl >= 0 ? PdfColor.fromHex('1E40AF') : PdfColor.fromHex('B91C1C'),
        ]));
      }
  ...
  static pw.Widget _buildPdfRegularRow(List<String> values, {bool isHeader = false, bool isBold = false, List<PdfColor?>? colors}) {
    return pw.Container(
      decoration: pw.BoxDecoration(
          ...
          return pw.Expanded(
            child: pw.Text(
              ...
              style: pw.TextStyle(
                ...
                color: colors != null && i < colors.length && colors[i] != null ? colors[i]! : (isHeader ? PdfColors.grey700 : PdfColors.black),
              ),
            ),
          );
        }).toList(),
  ...
  static Future<void> exportAsExcel(BillGenerateReport report, {String? billType}) async {
  ...
  static void _buildAdvanceExcelBody(Sheet sheet, BillGenerateReport report) {
    sheet.appendRow([TextCellValue('Bill Report - ${report.headerInfo.userName}')]);
    ...
    for (final trade in report.scriptTrades) {
      ...
      sheet.appendRow([TextCellValue('Date'), TextCellValue('Qty'), TextCellValue('Price'), TextCellValue('Volume')]);
      for (final leg in trade.buyLegs) {
        sheet.appendRow([TextCellValue(leg.date), DoubleCellValue(leg.qty.toDouble()), DoubleCellValue(double.tryParse(leg.price) ?? 0.0), DoubleCellValue(leg.vol)]);
      }
    ...
    sheet.appendRow([TextCellValue('Exchange'), TextCellValue('Script'), TextCellValue('Total(MTM)'), TextCellValue('Brokerage'), TextCellValue('Net Amount')]);
    for (final s in report.scriptWiseSummary) {
      sheet.appendRow([TextCellValue(s.exchange), TextCellValue(s.script), DoubleCellValue(s.total), DoubleCellValue(s.brokerage), DoubleCellValue(s.net)]);
    }
    sheet.appendRow([TextCellValue('TOTAL'), TextCellValue(''), DoubleCellValue(report.summaryTotal.total), DoubleCellValue(report.summaryTotal.totalBrokerage), DoubleCellValue(report.summaryTotal.totalNet)]);
  }
  static void _buildRegularExcelBody(Sheet sheet, BillGenerateReport report) {
    ...
    sheet.appendRow([TextCellValue('BILL SUMMARY ${report.headerInfo.dateRange.toUpperCase()}')]);
    sheet.appendRow([TextCellValue('')]);
    ...
    for (final trade in report.scriptTrades) {
      ...
      for (int i = 0; i < maxLen; i++) {
        ...
        sheet.appendRow([
          ...
          buyLeg != null ? DoubleCellValue(buyLeg.qty.toDouble()) : TextCellValue(''),
          buyLeg != null ? DoubleCellValue(double.tryParse(buyLeg.price) ?? 0.0) : TextCellValue(''),
          sellLeg != null ? DoubleCellValue(sellLeg.qty.toDouble()) : TextCellValue(''),
          sellLeg != null ? DoubleCellValue(double.tryParse(sellLeg.price) ?? 0.0) : TextCellValue(''),
          i == 0 ? DoubleCellValue(trade.brokerage) : TextCellValue(''),
          i == 0 ? DoubleCellValue(trade.profitLoss) : TextCellValue(''),
        ]);
      }
    }
    sheet.appendRow([TextCellValue('TOTAL'), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), DoubleCellValue(report.summaryTotal.totalBrokerage), DoubleCellValue(report.summaryTotal.total)]);
    sheet.appendRow([TextCellValue('')]);
    sheet.appendRow([TextCellValue('SCRIPT WISE SUMMARY')]);
    sheet.appendRow([TextCellValue('EXCHANGE'), TextCellValue('SCRIPT'), TextCellValue('MTM'), TextCellValue('BROKERAGE'), TextCellValue('NET AMOUNT')]);
    for (final s in report.scriptWiseSummary) {
      sheet.appendRow([TextCellValue(s.exchange), TextCellValue(s.script), DoubleCellValue(s.total), DoubleCellValue(s.brokerage), DoubleCellValue(s.net)]);
    }
    sheet.appendRow([TextCellValue('TOTAL'), TextCellValue(''), DoubleCellValue(report.summaryTotal.total), DoubleCellValue(report.summaryTotal.totalBrokerage), DoubleCellValue(report.summaryTotal.totalNet)]);
    ...
    if (report.carryForward.isNotEmpty) {
      ...
      sheet.appendRow([TextCellValue('EXCHANGE'), TextCellValue('SCRIPT'), TextCellValue('POSITION'), TextCellValue('QTY'), TextCellValue('RATE')]);
      for (final cf in report.carryForward) {
        sheet.appendRow([TextCellValue(cf.exchange), TextCellValue(cf.script), TextCellValue(cf.type.toUpperCase()), DoubleCellValue(cf.quantity), DoubleCellValue(cf.price)]);
      }
      sheet.appendRow([TextCellValue('')]);
    }
    if (report.exchangeWisePL.isNotEmpty) {
      sheet.appendRow([TextCellValue('EXCHANGE WISE SUMMARY')]);
      sheet.appendRow([TextCellValue('EXCHANGE'), TextCellValue('MTM'), TextCellValue('BROKERAGE'), TextCellValue('NET AMOUNT')]);
      for (final pl in report.exchangeWisePL) {
        sheet.appendRow([TextCellValue(pl.exchange), DoubleCellValue(pl.mtm), DoubleCellValue(pl.brok), DoubleCellValue(pl.pl)]);
      }
      sheet.appendRow([TextCellValue('TOTAL'), DoubleCellValue(report.summaryTotal.total), DoubleCellValue(report.summaryTotal.totalBrokerage), DoubleCellValue(report.summaryTotal.totalNet)]);
    }
  }
