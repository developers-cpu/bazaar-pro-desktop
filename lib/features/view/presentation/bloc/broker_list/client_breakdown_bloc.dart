...
class LoadClientBreakdownEvent extends ClientBreakdownEvent {
  ...
  const LoadClientBreakdownEvent({
...
class ClientBreakdownBloc
    extends Bloc<ClientBreakdownEvent, ClientBreakdownState> {
  final GetClientBreakdown getClientBreakdown;
  ClientBreakdownBloc({required this.getClientBreakdown})
    : super(ClientBreakdownInitial()) {
  ...
  Future<void> _onLoadClientBreakdown(
    ...
    try {
      final breakdown = await getClientBreakdown(
        event.brokerId,
        event.clientName,
      );
